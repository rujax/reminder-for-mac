...
struct WeeklyTab: View {
    ...
    private let days: [String] = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"]
    ...
    var body: some View {
        ...
        .padding(10)
        .onAppear() {
            print("WeeklyTab onAppear")

            title = reminder.title

            let timeArray = Array(reminder.time)

            selectedDay = days.firstIndex(of: String(timeArray[0..<2])) ?? 0
            selectedHour = Int(String(timeArray[2..<4])) ?? 0
            selectedMinute = Int(String(timeArray[5..<7])) ?? 0
        }
    }
}
